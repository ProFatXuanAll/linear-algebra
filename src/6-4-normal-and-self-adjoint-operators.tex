...
\begin{proof}[\pf{ex:6.4.14}]
  ...
  Let \(\lambda \in \R\) be an eigenvalue of \(\T\) and let \(\vs{E}_{\lambda}\) be the eigenspace of \(\lambda\).
  Now we split into two cases:
  \begin{itemize}
    \item If \(\vs{E}_{\lambda} = \V\), then any basis for \(\V\) over \(\R\) is consist of eigenvectors of \(\T\).
    ...
    \item If \(\vs{E}_{\lambda} \neq \V\), then by \cref{5.7} we have \(1 \leq \dim(\vs{E}_{\lambda}) < n + 1\).
          ...
          Since \(\T, \U\) are self-adjoint, by \cref{ex:6.4.7}(b) we know that \(\vs{E}_{\lambda}^{\perp}\) is both \(\T\)- and \(\U\)-invariant.
          By \cref{6.7}(c) we know that \(\dim(\vs{E}_{\lambda}^{\perp}) < n + 1\).
          Thus by induction hypothesis we can find some orthonormal bases \(\beta_1\) and \(\beta_2\) for \(\vs{E}_{\lambda}\) and \(\vs{E}_{\lambda}^{\perp}\) over \(\R\), respectively, consist of eigenvectors of both \(\T\) and \(\U\).
