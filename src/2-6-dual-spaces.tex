...
\begin{eg}\label{2.6.2}
  ...
  \[
    h(x) = \dfrac{1}{2 \pi} \int_{0}^{2 \pi} x(t) g(t) \; dt
  \]
...
\begin{proof}[\pf{2.6.2}]
  ...
  Then we have
  \begin{align*}
    h(cf_1 + f_2) & = \dfrac{1}{2 \pi} \int_{0}^{2 \pi} (cf_1 + f_2)(t) g(t) \; dt                                              &  & \by{2.6.2} \\
                  & = \dfrac{1}{2 \pi} \int_{0}^{2 \pi} cf_1(t) g(t) + f_2(t) g(t) \; dt                                                        \\
                  & = \dfrac{c}{2 \pi} \int_{0}^{2 \pi} f_1(t) g(t) \; dt + \dfrac{1}{2 \pi} \int_{0}^{2 \pi} f_2(t) g(t) \; dt                 \\
...
\begin{proof}[\pf{2.24}]
  Let \(g \in \V^*\).
  Since \(\dim(\V^*) = n\), we only need to show that
  \[
    g = \sum_{i = 1}^n g(x_i) f_i,
  \]
...
\begin{ex}\label{ex:2.6.10}
  ...
  \begin{enumerate}
    ...
    \item For any scalars \(\seq{a}{0,,n} \in \F\) (not necessarily distinct), deduce that there exists an unique polynomial \(q(x)\) of degree at most \(n\) such that \(q(c_i) = a_i\) for \(0 \leq i \leq n\).
          In fact,
          \[
    ...
    \item Prove that
          ...
          Suppose now that
          \[
            c_i = a + \dfrac{i (b - a)}{n} \quad \text{for } i \in \set{0, \dots, n}.
          \]
...
\begin{proof}[\pf{ex:2.6.10}(c)]
  ...
  For any scalars \(\seq{a}{0,,n} \in \F\), we can define \(q \in \ps[n]{\F}\) as follow:
  \[
