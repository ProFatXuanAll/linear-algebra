...
% Continuous function space over matric space #1.
\newcommand{\cfs}[1]{\vs{C}\p{#1}}

% Sequence.
% #1 is a comma-separated list of sequence symbols.
% #2 is a comma-separated list of sequence index.
% #3 is a join operator.
%
% For example:
% If we use \(\fseq{a}{1,2,3}{,}\), we will get
%
%    a_{1}, a_{2}, a_{3}
%
% If we use \(\fseq{a,b}{1,2,3}{+}\), we will get
%
%    a_{1} b_{1} + a_{2} b_{2} + a_{3} b_{3}
%
% If we use \(\fseq{a}{1}{,}\), we will get
%
%    a_{1}
%
% If we use \(\fseq{a}{1,2,,n}{,}\), we will get
%
%    a_{1}, a_{2}, \dots, a_{n}
%
\newcommand{\seq}[3]{%
	\setsepchar{,}%                              List items are separated by comma.
	\readlist\SeqSymbols{#1}%                    Define macro `\SeqSymbols' using #1.
	\readlist\SeqIndices{#2}%                    Define macro `\SeqIndices' using #2.
	\foreachitem\SeqIndex\in\SeqIndices{%        Loop over indices.
		\ifthenelse{%------------------------------IF-BEGIN------------------------
			\equal{\SeqIndex}{\SeqIndices[1]}%       No join operator for first index.
		}%-----------------------------------------IF-END--------------------------
		{%-----------------------------------------THEN-BEGIN----------------------
		...
		{%-----------------------------------------ELSE-BEGIN----------------------
			\ifthenelse{%--------------------------------IF-BEGIN--------------------
				\equal{\SeqIndex}{}%                   Replace empty item with `\dots'.
			}%-------------------------------------------IF-END----------------------
			{%-------------------------------------------THEN-BEGIN------------------
				#3\dots%
			}%-------------------------------------------THEN-END--------------------
			{%-------------------------------------------ELSE-BEGIN------------------
				#3%                                    Each non-first item in the list will prepend one join operator.
				\foreachitem\SeqSymbol\in\SeqSymbols{% Loop over symbols.
					{\SeqSymbol}_{\SeqIndex}%            Output symbols with index.
				}%
			}%-------------------------------------------ELSE-END--------------------
