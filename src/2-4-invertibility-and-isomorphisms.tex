...
\begin{proof}[\pf{2.20}]
  ...
  This is accomplished if we show that for every \(A \in \ms\), there exists an unique linear transformation \(\T : \V \to \W\) such that \(\Phi(\T) = A\).
  Let \(\beta = \set{\seq{v}{1,,n}}\), \(\gamma = \set{\seq{w}{1,,m}}\), and let \(A \in \ms\).
  By \cref{2.6} there exists an unique linear transformation \(\T : \V \to \W\) such that
  \[
...
\begin{proof}[\pf{ex:2.4.23}]
  ...
  This can be done by showing that every \(f \in \ps{\F}\) there exists an unique \(\set{b_n} \in \V\) such that \(\T(\set{b_n}) = f\).
  Let \(f \in \ps{\F}\).
