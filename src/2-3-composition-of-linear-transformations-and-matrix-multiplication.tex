...
\begin{proof}[\pf{ex:2.3.14}(a)]
  We have
  \begin{align*}
    Bz & = B \pa{\sum_{j = 1}^p a_j e_j} &  & \text{(by \cref{1.6.3})} \\
       & = \sum_{j = 1}^p (a_j B e_j)    &  & \text{(by \cref{2.3.5})} \\
       & = \sum_{j = 1}^p a_j v_j.       &  & \text{(by \cref{2.13})}
  \end{align*}
\end{proof}
...
\begin{proof}[\pf{ex:2.3.15}]
  ...
  \begin{align*}
    \begin{pmatrix}
      ...
      \vdots     \\
      (MA)_{p j}
    \end{pmatrix} & = M v_j = M \pa{\sum_{i = 1}^p c_i v_i} &  & \text{(by \cref{2.13}(a))}  \\
                    & = \sum_{i = 1}^p c_i (M v_i)            &  & \text{(by \cref{2.3.5})}  \\
...
\begin{proof}[\pf{ex:2.3.16}(a)]
  ...
  \begin{align*}
             & x \in \ns{\T}                                                                                     \\
    \implies & \T(x) = \zv                                                      &  & \text{(by \cref{2.1.10})}   \\
    \implies & \T\pa{\sum_{i = 1}^n a_i v_i} = \sum_{i = 1}^n a_i \T(v_i) = \zv &  & \text{(by \cref{2.1.2}(d))} \\
...
\begin{proof}[\pf{ex:2.3.17}]
  ...
  \begin{align*}
    ...
    \implies & \forall x \in \V, \T(x - \T(x)) = \T(x) - \T(\T(x)) &  & \text{(by \cref{2.1.2}(c))}     \\
             & = \T(x) - \T(x) = \zv                                                                    \\
